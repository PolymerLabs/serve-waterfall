 ...
 */
function waterfallServe(mappings, options) {
  options = options || {};
  var waterfall = _buildWaterfall(mappings, path.resolve(options.root || '.'));

  return function waterfallServeMiddleware(request, response, next) {
    ...
    _tryStep(waterfall, 0, request, response, next, function(remainder, target) {
 ...
 */
function _tryStep(waterfall, index, request, response, next, doSend) {
  ...
  if (requestPath.indexOf(step.prefix) !== 0) {
    return _tryStep(waterfall, index + 1, request, response, next, doSend);
  }
  // We have a match!
  ...
  doSend(remainder, step.target)
      .on('error', function(error) {
        ...
        _tryStep(waterfall, index + 1, request, response, next, doSend);
      })
      .pipe(response);
}
